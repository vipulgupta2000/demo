...
//added in v1.5 to get bonus from loaded payroll
$fname=$fname.",".'performance_linked_incentives';$plib=round($_POST['bonus'.$i]);$tot=$tot+$plib;
$b=$b.",".$plib;
}
...
//v1.6 changed $projected from $tot to also consider ytd for employees end dated
if($row['edate']<$dbdate1){$projected=projected($empid,$off,$payr,0,$tot); }else{ $projected=projected($empid,$off,$payr,$row['monthly_gross'],$tot);}
//v1.6 added this to remove arrear deduction from projected income
$arrded=isset($_POST['arrear_deduction'.$i])?round($_POST['arrear_deduction'.$i]):0;
  $projected=$projected-$arrded; 
$b=$b.",".$projected;
//Savings declaration
...
$fname=$fname.",".'taxable_income';$taxable_income=round($projected-$sav);
//New Code for Non Negative
...
$fname=$fname.",".'incometax_month';$tax_month=round(($proj_tax-$ytdtax)/($off+1));
//New Code for Non Negative
$tax_month =($tax_month > 0)?$tax_month:0;
$ded=$ded+$tax_month;$b=$b.",".$tax_month;
...
$sql_qual_query_m = mysql_query("select month from paystate where status='Open'");
		$curr_month =mysql_result($sql_qual_query_m,0); 
		$curr_year = date('Y');
...
$select_proj = mysql_query("select project_allowance from master where empid = ".$empid." and valid = 0");
  ...
  $select_proj1 = mysql_query("select project_allowance from master where id=".$v_id);
  $result_proj1 = mysql_result($select_proj1,0);
  $project_allowance_a = round(($days_pay/$mdays_tot)*$result_proj1);
  $project_allowance_a= ($project_allowance_a*$month_loop);
echo "projallowance".$project_allowance_a."p".$project_allowance_p;
	...
	$quer="update loaded_payroll set arrear_income = $arrear_income, arrear_deduction = $arrear_deduction where empid = $empid and month = $curr_month and year = $curr_year";
                echo $quer;
                mysql_query($quer);	
