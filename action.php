 ...
 * @author vipulgupta
 */
function action($tbl2,$field_edit,$field_show){
    $tbl=$tbl2;
    //for data access
...
if ($act=='click' ) {	
     echo "uid is".$uid;
    echo "rowaccess is".rowaccess($uid,$tbl);
    echo "access is".$_SESSION['SESS_access'];
	if(rowaccess($uid,$tbl)==1)
        {echo input_new($tbl,$sel,$field_edit,$field_show,2); }
        else
            echo "you dont have access to the record";
        $act="";
...
elseif (isset($_POST['updates'])) {  
     echo input_new($tbl,$sel,$field_edit,$field_show,2); 
if(isset($_POST['status0']) && $_POST['status0']=='Open')echo "be happy";
      // rowaccess($sel,$tbl);
